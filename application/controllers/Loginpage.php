...
class Loginpage extends CI_Controller{
    ...
    public function registerdata() {
        ...
        if ($this->input->method() == "post") {
            ...
            // Validation hatalarını kontrol ediyoruz
            if ($this->form_validation->run() === FALSE) {
                $error = array(
                    'title' => 'Hata',
                    'text'  => 'Boş alan bıraktınız veya geçersiz format...',
                    'icon'  => 'error'
                );
                echo json_encode($error);  // JSON olarak hatayı döndürüyoruz
                return;
            }
    
    
            // Eğer kullanıcı zaten varsa, JSON ile hata mesajı döndür
            ...
            // Veritabanı işlemi başarılı olursa
            $userdata = array(
                'uye_kodu'       => uniqid(),

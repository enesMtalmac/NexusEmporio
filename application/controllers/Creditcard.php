    ...
    public function resultpaytr(){
        ...
        if( $post['status'] == 'success' ) { ## Ödeme Onaylandı
            ...
            if($setting->mailbildirim == 1){
                ...
                $mail->Username    = $smtpinfo->smtp_user;
                $mail->Password    = $smtpinfo->smtp_pass;
                $mail->IsSMTP();
                $mail->SMTPAuth    = true;
                $mail->CharSet     = "UTF-8";

                $mail->SetFrom($smtpinfo->smtp_user, $setting->site_baslik);
                $mail->AddAddress($userquery->uyeeposta);
                $mail->Subject = 'Sipariş Bilgileri - '.date("d.m.Y");

                $mailBody = 'Sayın '.$userquery->uye_adi.', siparişiniz başarıyla alınmıştır. Sipariş kodunuz: '.$orderno;
                $mail->Body = $mailBody;
            ...
            redirect(base_url('odeme/siparislerim'));
        } else { ## ödeme başarısız
            redirect($pos->hataurl);
        }
    }
